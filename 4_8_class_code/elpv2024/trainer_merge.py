...
class Trainer:
    ...
    # trainer_merge.py 里替换 save_checkpoint / restore_checkpoint（最小改动版本）
    ...
    def save_checkpoint(self, epoch):
        ...
        t.save({
            ...
            'epoch': epoch
        }, os.path.join(CHECKPOINTS_DIR, f'checkpoint_{epoch:03d}.ckp'))

    def restore_checkpoint(self, epoch_n):
        ckp = t.load(os.path.join(CHECKPOINTS_DIR, f'checkpoint_{epoch_n:03d}.ckp'),
                     'cuda' if self._cuda else 'cpu')
        self._model.load_state_dict(ckp['state_dict1'])
        self._model2.load_state_dict(ckp['state_dict2'])
    ...
    def train_epoch(self, epoch,alpha=1.0):
    ...
    def calculate_metrics(self, total_loss, y_preds, y_grounds):
        ...
        f1_each = f1_score(y_grounds, y_preds, average=None, labels=labels, zero_division=0)
        p_each = precision_score(y_grounds, y_preds, average=None, labels=labels, zero_division=0)
        r_each = recall_score(y_grounds, y_preds, average=None, labels=labels, zero_division=0)
        ...
        # print("F1 mean: ", f1_mean,f1_mean_un,f1_mean_de1,f1_mean_de2, f1_mean_de3, "\n")
        print("F1 mean: ", f1_mean, f1_each, "\n")
        print("ACC: ", acc, "\n")
        print("BALANCE ACC: ", balanced_acc, "\n")
        print("PRECISION: ", precision,p_each,"\n")
        # print("PRECISION: ", precision, precision_un, precision_de1, precision_de2, precision_de3, "\n")
        print("RECALL: ", recall, r_each, "\n")
        # print("RECALL: ", recall,recall_un,recall_de1,recall_de2,recall_de3, "\n")
        print('##############################\n')
    ...
    def fit(self, epochs=-1,args=None):
        ...
        while True:
            ...
            print("--- Epoch", epoch, "---")
            train_lo, train_lo2 = self.train_epoch(epoch,alpha=args.alpha)
